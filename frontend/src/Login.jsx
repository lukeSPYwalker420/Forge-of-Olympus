...
export default function Login() {
  ...
  const handleEmailChange = (e) => {
    ...
    // Check if this email is admin (hardcoded or you can make it dynamic)
    const adminEmail = "kieren2203@googlemail.com";
    if (value.toLowerCase() === adminEmail.toLowerCase()) {
      setShowPassword(true);
    } else {
      setShowPassword(false);
      setPassword(""); // Clear password when switching from admin
    }
  };
  ...
  const handleLogin = async () => {
    ...
    // If password field is visible (admin), require password
    ...
    try {
      const res = await fetch("/api/login", {
        ...
        body: JSON.stringify({ 
          ...
          password: password  // Send password (empty for non-admin)
        })
      });
  ...
  return (
    <div style={{ height: "100vh", display: "flex", justifyContent: "center", alignItems: "center", fontFamily: "system-ui", background: "#0a0c10" }}>
      <div style={{ padding: 30, border: "1px solid #333", borderRadius: 12, width: 320, background: "#1e1e2a" }}>
        ...
        {error && (
          <div style={{ color: "#ff5555", fontSize: 14, marginBottom: 12, textAlign: "center" }}>
            {error}
          </div>
        )}
