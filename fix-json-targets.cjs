...
const accessoryKeywords = [
  ...
  'incline', 'decline', 'dip', 'pull apart', 'band', 'press'
];
...
const mainLiftKeywords = [
  ...
  'front squat', 'trap bar deadlift', 'zercher', 'close grip bench',
  'paused squat', 'bench paused', 'rdl', 'romanian deadlift'
];
...
function isAccessory(liftName, role) {
  ...
  if (role === 'top-set' || role === 'back-off' || role === 'secondary' || role === 'primary') return false;
...
files.forEach(file => {
  ...
  data.sessions.forEach(session => {
    ...
    session.exercises.forEach(ex => {
      ...
      // Assign targets based on progression type
      if (ex.progressionType === 'strength') {
        if (ex.rpeTarget === undefined || ex.rpeTarget === null) {
          if (isDeloadWeek) {
            ex.rpeTarget = 4;
          } else if (isAcc) {
        ...
        if (ex.rirTarget === undefined || ex.rirTarget === null) {
          if (isDeloadWeek) {
            ex.rirTarget = 4;
          } else if (isAcc) {
        ...
        if (ex.qualityTarget === undefined || ex.qualityTarget === null) {
          ex.qualityTarget = isDeloadWeek ? 6 : 8;
          modified = true;
        }
      }
      
      else if (ex.progressionType === 'mobility' || ex.progressionType === 'stability') {
        if (ex.stabilityTarget === undefined || ex.stabilityTarget === null) {
          ex.stabilityTarget = isDeloadWeek ? 6 : 7;
          modified = true;
        }
        if (ex.painTarget === undefined || ex.painTarget === null) {
          ex.painTarget = isDeloadWeek ? 3 : 4;
          modified = true;
        }
      }
